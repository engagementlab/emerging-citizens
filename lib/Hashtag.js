...
module.exports = function(gameSession) {
    ...
    function StartVoting(socket) {
        ...
        _.each(_.keys(_PLAYERS), function(id, index) {
            ...
            if(_CURRENT_SUBMISSIONS[id] === undefined)
                _CURRENT_SUBMISSIONS[id] = { hashtag: Sentencer.make('{{noun}}'), username: _PLAYERS[id].username };

        });

        // Add real hashtag
        _CURRENT_SUBMISSIONS[realId] = { hashtag: _REAL_HASHTAG.replace('#', ''), real: true };
    ...
    this.HashtagSubmitted = function(player, submission, socket, space) {
        ...
        _CURRENT_SUBMISSIONS[player] = { hashtag: thisHashtag, username: _PLAYERS[player].username };

        socket.emit('hashtag:success');

        // All hashtags received; voting begins
        if(Object.keys(_CURRENT_SUBMISSIONS).length === currentPlayerCap)
            StartVoting(space);
    ...
    this.HashtagVote = function(playerId, submitterId, socket) {
        ...
        var thisHashtag = _CURRENT_SUBMISSIONS[submitterId];
