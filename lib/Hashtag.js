...
module.exports = function(gameSession) {
    ...
    function StartVoting(socket) {
        ...
        // console.log (JSON.stringify(_PLAYERS) + " are the players");
        _.each(_PLAYERS, function(playerId, index) {
            // console.log (JSON.stringify(index) + " player ID");
            // console.log (JSON.stringify(_PLAYERS_SUBMITTED) + " are the current submissions");
            // console.log (_.indexOf(_PLAYERS_SUBMITTED, index));
            if ((_.indexOf(_PLAYERS_SUBMITTED, index)) === -1) {
                console.log ("found player with no submission");
                var randomHashtag = Sentencer.make('{{noun}}');
                _CURRENT_SUBMISSIONS[randomHashtag] = [ { id: index, username: _PLAYERS[index].username } ];
            }    
        });
        ...
        _CURRENT_SUBMISSIONS[realHashtag] = [{ id: realId, real: true }];
        // console.log (JSON.stringify(_CURRENT_SUBMISSIONS) + " are the current submissions");

        // Convert to array for shuffling
        randomizedHashtags = _.pairs(_CURRENT_SUBMISSIONS);
        // Randomize the hashtag order and make sure all displayed ones for group view are unique
        randomizedHashtags = _.shuffle(randomizedHashtags);
        // console.log (JSON.stringify(randomizedHashtags) + " are the current submissions");

        // Send the hashtags to the voting screen
        randomizedHashtags = _.values(randomizedHashtags)
    ...
    function ShowSubmissions(socket) {
        ...
        var data = {   
                   ...
                   console.log (JSON.stringify(data) + " is the goddamn data");
    ...
    this.HashtagSubmitted = function(player, submission, socket, space) {
        ...
        //if duplicate hashtag, add to that hashtag array in _CURRENT_SUBMISSIONS object
        if (_.has(_CURRENT_SUBMISSIONS, thisHashtag)){
            
            _CURRENT_SUBMISSIONS[thisHashtag].push( {id: player, user: _PLAYERS[player]} );
            // console.log (JSON.stringify(_CURRENT_SUBMISSIONS) + " are the current submissions");
        } else {
            _CURRENT_SUBMISSIONS[thisHashtag] = [{id: player, user: _PLAYERS[player]}];
        }
        ...
        // Find number of users that have submitted
        for (var key in _CURRENT_SUBMISSIONS) {
            var result = _.pluck(_CURRENT_SUBMISSIONS[key], 'id');
            // console.log (result + " is an ID");
            if (!(_.has(_PLAYERS_SUBMITTED, result))) {
                // console.log ("pushing to _PLAYERS_SUBMITTED")
                _PLAYERS_SUBMITTED.push(result);
            }
        }
        _PLAYERS_SUBMITTED = [].concat.apply([], _PLAYERS_SUBMITTED);
        // console.log(JSON.stringify(_PLAYERS_SUBMITTED) + " are the concatted ID's");

        _PLAYERS_SUBMITTED = _.uniq(_PLAYERS_SUBMITTED);

        // console.log(JSON.stringify(_PLAYERS_SUBMITTED) + " are the Uniq ID's for the players");
    ...
    this.HashtagVote = function(playerId, hashtag, socket) {
        // console.log(JSON.stringify(_CURRENT_SUBMISSIONS) + " are the current submissions");
        var hashtagSubmissions = _.pairs(_CURRENT_SUBMISSIONS);
        // console.log(JSON.stringify(hashtagSubmissions) + " are the new submissions");
        
        var submitterId = [];
        console.log(JSON.stringify(randomizedHashtags));
        console.log (hashtag);
        _.each(randomizedHashtags, function(value, index) {
            console.log (value + " is the value, and the index is: " + index);
        });
        var thisHashtag = randomizedHashtags[hashtag];
                console.log (JSON.stringify(thisHashtag) + " was voted for!");

        for (var item of hashtagSubmissions) {
            // console.log (thisHashtag['hashtag'] + " is a submission");
            if (item[0] === thisHashtag['hashtag']){
                console.log ("found it");
                // console.log (JSON.stringify(item[1]) + " are the players who submitted that hashtag");
                var submitterId = _.pluck(item[1], 'id');
            }
        }
        console.log (submitterId + " submitted that hashtag!")
        var voteScore = 0;
        ...
        if(thisHashtag.real)
            ...
            for (var player of submitterId){
                // console.log (player);
                voteScore = CalcPlayerScore(player, false); 
            }
            
            console.log (thisHashtag['votes'] + " is the hashtag votes");
        // Save vote for hashtag
        if(thisHashtag['votes'] === undefined) {

            thisHashtag['votes'] = [ { 
                                    user: _PLAYERS[submitterId],
                                    score: voteScore
                                } ];
