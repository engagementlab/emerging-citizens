...
module.exports = function(gameSession) {
    ...
    function StartVoting(socket) {
        ...
        Templates.Load('partials/moderator/voting', undefined, function(html) {
    ...
    function ShowSubmissions(socket) {
        ...
        // Hashtags are spliced into two arrays for column layout
        var hashtagsLengthHalf = Math.ceil(_CURRENT_HASHTAGS.length / 2);
        var data = {   
                        hashtags: _CURRENT_HASHTAGS.splice(0, hashtagsLengthHalf), 
                        tweet: _CURRENT_TWEET,
                        players: _.sortBy(_PLAYERS, function(player){ return Math.sin(player.score_total); })
                   };

        Templates.Load('partials/moderator/submissions', data, function(html) {
