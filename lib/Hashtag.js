...
module.exports = function(gameSession) {
    ...
    function StartVoting(socket) {
        ...
        var randomizedHashtags = _.shuffle(_CURRENT_HASHTAGS);
        var data = {   
    ...
    this.HashtagSubmitted = function(player, submission, socket, space) {
        ...
        _CURRENT_HASHTAGS[player] = { hashtag: thisHashtag, username: _PLAYERS[player].username };

        socket.emit('hashtag:success');

        // All hashtags received; voting begins
        if(Object.keys(_CURRENT_HASHTAGS).length === currentPlayerCap)
            StartVoting(space);
