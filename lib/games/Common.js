...
class Common {
    ...
    Countdown(socket, data) {
        ...
        if (this._game_session.gameType === "wikigeeks") {
            timeLeft = this._config.timeLimitWiki * 60;
            // console.log (timeLeft, "time left");
        }
        if (this._game_session.gameType === "htyi") {
            timeLeft = this._config.timeLimitHashtag * 60;
            // console.log (timeLeft, "time left");
        }
        ...
        this._countdown = setInterval(() => {
            ...
            if(timeLeft === 0) {
                console.log (timeLeft, "timeLeft is 0")
                clearInterval(this._countdown);
    ...
    AdvanceRound(socket) {
        ...
        this._players_submitted = [];
        console.log (this._players_submitted, "submitted players");
        _.each(this._currentPlayers, (playerId, index) => {

            this._currentPlayers[index] = _.omit(this._currentPlayers[index], 'score');
            this._currentPlayers[index].submitted = false;
