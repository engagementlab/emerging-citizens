...
class MemeLib extends Core {
    ...
    MemeSubmitted(playerSocketId, submission, socket) {
        ...
        console.log('meme:received', thisPlayer.uid)

        // Let player know submission received w/ their uid
        // and tell group screen the index of submitter
        socket.emit('meme:received', thisPlayer.uid);
        this._group_socket.to(this.group_id).emit('meme:received', {index: thisPlayer.index});
    ...
    DisplayResults(force) {
        ...
        console.log(this._current_submissions, "these are the current submissions okay ");
        let submissionVotes = [];
        ...
        console.log(submissionVotes, "this is the supposed voted memesss");
        ...
        if(lastRound) {
            playerData.survey = this._config.surveyMeme;
            playerData.surveyUrl = this._config.surveyUrlMeme;
        }
