...
class WikiLib extends Common {
            ...
            this.Templates.Load('partials/group/wiki/topic', data, (html) => {
                ...
                this.eventEmitter.once('countdownEnded', (data, socket) => {
                    ...
                    if (this.playersSubmitted.length < this.currentPlayerCap) {
                        // console.log ("someone didnt submit!");
                        _.each (this._current_players, (player, index) => {
                            if (player.submitted === false) {
                                console.log (player, "didn't submitt!!!!!!!!");
                                var apiUrl = 'https://en.wikipedia.org/w/api.php?format=json&action=query&format=json&list=random&rnnamespace=0&rnfilterredir=nonredirects&rnlimit=1';
                                 this.request(apiUrl, (error, response, body) => {
                              
                                    if (!error && response.statusCode == 200){}

                                    let article = JSON.parse(body).query.random[0].title;
                                    console.log (article);
                                    this.PlayerClick (player.socket_id, article, true);
                                    console.log (player.socket_id, this._current_players);
                                    this.groupSocket.to(player.socket_Id).emit('article:random', article);
    ...
    DisplayArticle(playerSocketId) {
        ...
        console.log (this.currentPlayerCap, "versus", this.playersSubmitted.length)
    ...
    PlayerClick(playerSocketId, articleTitle, initialSearch) {
        ...
        if(initialSearch) {
            ...
            // console.log (_.values(this._current_players));
            
            console.log (this._current_players);


            this.playersSubmitted.push(playerSocketId);

            console.log (this.playersSubmitted, "players");
            console.log (this._countdown_duration, "countdown");

            if (this._countdown_duration > 5 && this._countdown_duration !== this._config.timeLimitWiki){
        ...
        if(playerSubmission === this.allArticles[this._current_round].articleName) {
            ...
            this.Templates.Load('partials/player/wiki/results', undefined, (html) => {
                ...
                this.groupSocket.to(playerSocketId).emit('wiki:results', thisPlayerProgress);
