...
class WikiLib extends Common {
        ...
        super.Initialize(gameSession, (config) => {
            ...
            this.Links.model.find({}, (err, result) => {
                ...
                _.each (topics, (topic, index) => {
                    // topics [topic] 
                    console.log (index, "index", topic, "this topics");
                    var name = topic.category["name"];
                    var link = _.shuffle(topic.destinationLinks)[0];
                    // let topic = topics.
                    this._all_articles.index = { name : link} ;
                   
                });

                // _.each (_all_articles, (article, index) => {
    ...
    StartGame(socket) {
        ...
        Templates.Load('partials/group/wiki/article', undefined, (html) => {
            socket.to(this.group_id).emit('article:start', html);
        });

        Templates.Load('partials/player/wiki/article', undefined, (html) => {
