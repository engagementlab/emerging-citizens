...
class WikiLib extends Common {
    ...
    constructor() {
        ...
        this.wikipediaApi = require("wikipedia-js"),
        ...
        super.Initialize(gameSession, (config) => {
            ...
            this.Links.model.find({}, (err, result) => {
                ...
                _.each (topics, (topic, index) => {
                    ...
                    var name = topic.category.name;
                    var image = topic.category.image;
                    var description = topic.category.description.html;
                    var link = _.shuffle(topic.destinationLinks)[0];

                    console.log (name, "name", index, "index");
                    
                    this._all_articles.push ({ "topicName": name, "topicDescription" : description, "link" : link, "image" : image })
                    // console.log (this._all_articles.index);                   
                });
    ...
    DisplayTopic(articleIndex, socket) {
        ...
        this._destination_article = data[this._topic];
    ...
    SearchArticles(socket, playerQuery) {
        ...
        var options = {query: playerQuery, format: "html", summaryOnly: false};
        
        this.wikipediaApi.searchArticle(options, function(err, htmlWikiText){
            ...
            Templates.Load('partials/player/article', data, (html) => {
                ...
                socket.to(this.players_id).emit('article:found', html);
