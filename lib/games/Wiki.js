...
class WikiLib extends Common {
    ...
    DisplayResults(socket) {
        ...
        let playerData = Object.assign({}, this.playerProgress[this._current_round]);

        _.each(playerData, (player, ind) => {

            console.log('player', player)

            // If player never finished, set their time to the entire duration
            if(!player.finished)
                player.time = this._config.timeLimitWiki;

            // Calc total score
            player.score_total = _.reduce(player.scores, function(memo, num){ return memo + num; });

        });

        this.Templates.Load('partials/group/wiki/results', {results: playerData}, (html) => {
    ...
    PlayerClick(playerSocket, articleTitle) {
        ...
        this._players_submitted.push (articleTitle);
        console.log (this._players_submitted, "players submitted");

        if(this._players_submitted.length === this.currentPlayerCap)
            this.DisplayArticle(articleData, playerSocket);
        ...
        if(!allPlayerProgress[this._current_round][playerSocket.id])
            allPlayerProgress[this._current_round][playerSocket.id] = { user: thisPlayer, articles: [], scores: [0, 0, 0] };
        ...
        if(playerSubmission === this.allArticles[this._current_round].articleName) {
            ...
            console.log('%s players are done', _.pluck(allPlayerProgress, "finished").length);
