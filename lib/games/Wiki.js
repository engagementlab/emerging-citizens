...
class WikiLib extends Common {
    ...
    PlayerClick(playerSocketId, articleTitle, initialSearch) {
        ...
        if(playerSubmission === this.allArticles[this._current_round].articleName) {
            ...
            console.log (this._current_players, playerSocketId);
            ...
            thisPlayerProgress.scores[1] = (this._config.scorePlacementWiki * playerFinishedCt);
            ...
            // Are we waiting for one player to reach target article after time expiry? If so, go right to results
            if(this.waitingForFinish)      
                this.DisplayResults();
