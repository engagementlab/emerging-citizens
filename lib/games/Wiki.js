...
class WikiLib extends Common {
    ...
    DisplayTopic(articleIndex, socket) {
        ...
        data.timeLimit = this._config.timeLimitWiki;
        ...
        this.Templates.Load('partials/group/wiki/topic', data, (html) => {
            ...
            socket.to(this.group_id).emit('game:start', {timeLimit: this._config.timeLimitWiki, html: html});
            
            // Begin first countdown and assign end event
            this.Countdown(socket, data);
            
            this.eventEmitter.on('countdownEnded', (topicData, socket) => {
                this.DisplayArticle(topicData, socket);
            });  
    ...
    DisplayArticle(data, socket) {
        ...
        this.Templates.Load('partials/group/wiki/article', data, (html) => {
            ...
            this.eventEmitter.on('countdownEnded', (topicData, socket) => {
                this.SearchArticles(socket, topicData);
            });
    ...
    PlayerClick(playerSocket, articleTitle) {
        ...
        if(!playerProgress[_current_round])
            playerProgress[_current_round] = {};

        if(!playerProgress[_current_round][thisPlayer.username])
            playerProgress[_current_round][thisPlayer.username] = [articleTitle];
        else
            playerProgress[_current_round][thisPlayer.username].push(articleTitle);

        // Did player get to target article?
        if(articleTitle === this.allArticles[_current_round].articleName) {
            thisPlayer.finished = true;
            playerSocket.emit('article:target');
        }
