...
class WikiLib extends Common {
    ...
    AdvanceRound(socket) {
        ...
        // console.log(this.playerProgress, "is the player progress");
    ...
    DisplayTopic(socket) {
        ...
        this.Templates.Load('partials/group/wiki/topic', data, (html) => {
            ...
            this.eventEmitter.once('countdownEnded', (data, socket) => {
                console.log("the countdown ended...calling DisplayArticle()");

                this.checkSearching = setInterval( () => { this.CheckIfSearching() }, 1000);
    ...
    DisplayArticle(playerSocketId) {
        ...
        console.log("now displaying article...");
        ...
        let playerChecked = _.after(this.currentPlayerCap, () => {
            ...
            this.Templates.Load('partials/group/wiki/article', data, (html) => {
                ...
                console.log(this.playerFinishedCt, "finished number of players");
                if (this.playerFinishedCt > 0) {
                    for (var i=0; i <= this.playersFinishedCt; i++){
                        ...
                        console.log(finished, "is a player that finished already!")
                        this._group_socket.to(this.group_id).emit('player:finished', finished);
                    }
                }

                console.log(this._current_round);

                console.log("player progress" + this.playerProgress[this._current_round]);
        ...
        _.each (this._current_players, (player, index) => {
            console.log(player, "checking if this player submitted");
            if (!player.submitted) {
    ...
    PlayerClick(playerSocketId, articleTitle, initialSearch) {
        ...
        console.log(thisPlayer, "is the player that just submitted");
        ...
        if(playerSubmission === this.allArticles[this._current_round].articleName) {
            ...
            console.log ("player found the destination article");
            ...
            console.log(this.playerFinishedCt, "are the number of players finished");
    ...
    CheckIfSearching() {
        if (this.searching) {
            ...
            console.log("no one is searching..");
