...
class WikiLib extends Common {
    ...
    constructor() {
        ...
        this.wikipediaApi = require("node-wikipedia"),
    ...
    SearchArticles(socket, playerQuery) {
        ...
        var options = {query: playerQuery, format: "html", summaryOnly: true};
        
        // this.wikipediaApi.searchArticle(options, (err, resultHtml) => {
           
        //     if(err) {
        //       console.log("An error occurred[query=%s, error=%s]", playerQuery, err);
        //       return;
        //     }
            
        //     // console.log("Query successful[query=%s, html-formatted-wiki-text=%s]", playerQuery, html);

        // });

        this.wikipediaApi.page.data(playerQuery, { content: true }, (response) => {
            // structured information on the page for Clifford Brown (wikilinks, references, categories, etc.)
            // console.log('wiki', response)

            this.Templates.Load('partials/player/wiki/article', {html: response.text['*']}, (html) => {
