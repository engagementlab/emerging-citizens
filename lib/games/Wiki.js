...
class WikiLib extends Common {
    ...
    SearchArticles(socket, playerQuery) {
        ...
        this.request(
            extractUrl,
            (error, response, bodyExtract) => {
     
                if (!error && response.statusCode == 200) {
                    
                    let extractJson = JSON.parse(bodyExtract);
                    let articleIndex = Object.keys(extractJson.query.pages)[0];
                    let articleExtract = extractJson.query.pages[articleIndex].extract;
                    let articleId = extractJson.query.pages[articleIndex].pageid;
