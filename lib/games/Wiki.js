...
class WikiLib extends Core {
    ...
    Initialize(gameSession) {
        ...
        super.Initialize(gameSession, (config) => {
            ...
            // console.log(gameSession, " the game session categories right ehre");

            // Content buckets for this session

            var linkQuery = this.Links.model.find({})
                                                .populate("category");

            // this.Links.model.find({ 'category': { $in: gameSession.contentCategories } };
            
            linkQuery.exec( (err, result) => {

                console.log (result, " is the result");
                let links = [];
                _.each(result, (link, index) => {
                    console.log(link, "the link right here");
                    if (_.contains(gameSession.contentCategories, link.category)) {
                        links.push(link);
                    }
                    // links.push(_.where(link.category._id, {$in: gameSession.contentCategories}));
                });

                console.log (links, " is the links filtered by category");
                
                var roundlimit = config.roundNumber;

                // Randomize topics
                links = _.sample(shuffle(result), roundlimit);

                _.each (links, (link, index) => {

                    console.log(link.articleName, link.category)
